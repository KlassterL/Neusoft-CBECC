...
const saveProduct = () => {
    ...
    if (product.value.name && product.value.name.trim() && product.value.price && product.value.stock && product.value.category && product.value.category.trim()) {
        if (product.value.product_id) {
            mvoAPI.editProduct(product.value.product_id, product.value).then(res => {
                if (res) {
...
<template>
    <div class="grid">
        <div class="col-12">
            <div class="card">
                <DataTable :value="products" v-model:selection="selectedProducts" dataKey="product_id" :paginator="true" :loading="loading"
                    :rows="10" :filters="filters" :rowsPerPageOptions="[5, 10, 25]" currentPageReportTemplate=""
                        ...
                        <div class="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
                            ...
                            <div>
                                <span class="my-2">
                                    ...
                                    <Button icon="pi pi-trash" severity="danger" class="mr-3" @click="confirmDeleteSelected"
                    ...
                    <template #empty>
                        <div class="text-center text-xl">暂无旗下商品信息</div>
                    </template>
                    ...
                    <Column header="操作" headerStyle="min-width:10rem;">
                        <template #body="slotProps">
                            <Button icon="pi pi-pencil" severity="primary" label="修改" class="mr-2"
                ...
                <Dialog v-model:visible="productDialog" :style="{ width: '450px' }" header="商品详情" :modal="true"
                    class="p-fluid">
                    <img :src="product.image_url" v-if="product.image_url" width="150"
                    ...
                    <div class="field">
                        ...
                        <InputText id="category" v-model.trim="product.category" required="true"
                    ...
                    <div class="formgrid grid">
                        <div class="field col">
                            ...
                            <InputNumber id="price" v-model="product.price" mode="currency" currency="CNY" locale="zh"
                        ...
                        <div class="field col">
                            ...
                            <InputNumber id="stock" v-model="product.stock" integeronly required="true"
                    ...
                    <template #footer>
                        ...
                        <Button label="保存" icon="pi pi-check" severity="success" class="p-button-text"
                            @click="saveProduct" />
                    </template>
                </Dialog>
                ...
                <Dialog v-model:visible="deleteProductDialog" :style="{ width: '450px' }" header="商品删除" :modal="true">
                    <div class="flex align-items-center justify-content-center">
                        ...
                        <span v-if="product">你确定想要删除 <b>{{ product.name }}</b> 吗？</span>
                    </div>
                    <template #footer>
